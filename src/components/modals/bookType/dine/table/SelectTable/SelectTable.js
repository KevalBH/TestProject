...
function SelectTable({ restaurantId, onClose, onSetTableNo, selectedTable }) {

    const [tableData, setTableData] = useState([
        { text: 'cost per person: 120rs', seats: 4, vertical: false, position: { x: 0, y: 0 }, type: 4, selected: false },
        { text: 'cost per person: 220rs', seats: 6, vertical: true, position: { x: 220, y: 39 }, type: 4, selected: false },
        { text: 'cost per person: 320rs', seats: 4, vertical: false, position: { x: 460, y: 0 }, type: 0, selected: false },
    ])
    ...
    useEffect(() => {
        ...
        if (selectedTable.length > 0) {
            let a = selectedTable.split(', ')
            a = a.map((item) => Number(item.substr(1, item.length)) - 1)
            let b = tableData
            a.map((item) => {
    ...
    function onContinueClick() {
        let array = tableData.map((item, index) => {
            if (item.selected == true)
                return 'T' + (index + 1)
            else return '-'
        })
        array = array.filter((item) => item != '-')
        // console.log('continue click', array)
        onSetTableNo(array)
        onClose(false)
    }
    ...
    return <>
        ...
        <MyModal onOk={onContinueClick} okText={'Continue'} onCancel={() => onClose(false)} cancelText={'Back'} title="Select Table" visible={true} >
            ...
            <Container>

                {
                    tableData.map((item, index) => {
                        ...
                        if (item.vertical)
                            return <DiningTableV text={item.text} onClick={() => handleTableClick(index)} selected={item.selected} chairs={item.seats} type={item.type} id={index + 1} position={item.position} />
                        return <>
                            <DiningTableH text={item.text} onClick={() => handleTableClick(index)} selected={item.selected} chairs={item.seats} type={item.type} id={index + 1} position={item.position} />
                        </>
                    })
                }

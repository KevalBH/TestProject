...
function DiningTable(props) {
    ...
    // right side
    for (let i = 0, x = 0, y = baseMargin; i < lengthHalf; i++) {
        let a = {
            position: { x: x, y: y },
            view: POSITION.RIGHT
        }
        y += DIMENSION.H + spaceMargin
        chairsArray.push(a)
    }

    // left side
    for (let i = 0, x = w + DIMENSION.W - 18, y = baseMargin; i < lengthHalf; i++) {
        let a = {
            position: { x: x, y: y },
            view: POSITION.LEFT
        }
    ...
    useEffect(() => {
        ...
        if (type != TYPE.NONE) {
            ...
            ctx.fillRect(DIMENSION.W + w - 10, 4, 8, th - 9)
            ...
            h5.style.top = th - 20 - 10 + 'px'
            h5.style.left = DIMENSION.W + 10 + 'px'
        ...
        if (props.id != undefined) {
            ...
            h5.style.left = DIMENSION.W + 5 + 'px'
            h5.style.top = 5 + 'px'
    ...
    return <>
        <div onDragStart={() => ODS()} onClick={onClickToMain} draggable={isDraggable} className='dt_shape' key={'dining_table_' + props.id} id={'dining_table_' + props.id} style={{ filter: selected ? 'brightness(0.9)' : '', transform: `rotate(${rotate}deg)`, height: th + 1, width: DIMENSION.W * 2 + w, left: position.x, top: position.y, position: 'absolute' }} >
            {isDraggable && <SvgDrag />}
            <SvgToolTip message={text} />
            <div className='shape_table' id={'table' + props.id} style={{ zIndex: 0, position: 'absolute', width: '100%', height: '100%' }}>
                <canvas height={th} ></canvas>
            </div>
            {
...
function SvgDrag(props) {
    ...
    let style = { top: 0, left: 0, width: '15px', height: '15px', position: 'absolute' }
